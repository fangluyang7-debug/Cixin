...
与鸿蒙项目共用 App、调度 SDK、任务画像和云端业务链路的边界见 [与 Harmony 共享链路说明](docs/与Harmony共享链路说明.md)。Cixin 目录只增加 P1/多开发板适配，不维护第二套正式购物链路。
...
调度前实测网络，结合输入大小和执行历史参与时限判断与选路；网络切换、过期和失败触发失效。接入变化与验收见[网络画像驱动调度](docs/网络画像接入说明.md)。
