...
**手机验证入口（2026-09-21）：** 请按 [手机验证操作手册与结果回传](docs/11-手机验证操作手册与结果回传.md) 签名安装、执行真实实验并导出 JSON。App 已增加三策略/有无后台对照入口；电脑汇总使用 `scripts/summarize-phone-experiment.py`。主机测试与手机结论分开记录。
