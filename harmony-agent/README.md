...
**实时仪表盘与低频主观反馈（2026-09-22）：** 右上角“调度盘”在购物页内展示真实调度快照，不因查看而取消搜索。撤销首次内部参数题及发热/卡顿题；结合应用实测耗时等信息，仅在有必要时询问等待容忍度或结果是否符合需求，保留授权、限频及原策略门槛。HTML 仍为原型，没有电脑浏览器到手机的联网桥接。详见 [实时调度仪表盘与提问诊断](docs/13-实时调度仪表盘与提问诊断.md)。
