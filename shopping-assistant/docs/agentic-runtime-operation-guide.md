...
> 2026-09-15 规范化状态：本文件是唯一维护的操作指南，外层旧副本已删除。核心默认无业务模板，演示层注册购物规则；其他应用可注册自己的操作、Provider v1 和设备适配器。预测支持 warning，终端保护与检查点关联接口已实现，但无训练模型、真实购物 Handler 或板端完整验证。详见 [功能说明](../../../docs/调度件初版功能与端侧模型接入.md) 与 [测试报告](../../../docs/调度件规范化测试报告.md)。下文早期路线图保留为历史背景，完成状态以上述文档为准。
